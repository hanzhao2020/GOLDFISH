...
class MaxvMStressModel(Model):
    def initialize(self):
        self.parameters.declare('nonmatching_opt')
        self.parameters.declare('rho', default=1.)
        self.parameters.declare('alpha', default=None)
        self.parameters.declare('m', default=None)
        self.parameters.declare('surf', default='top')
        self.parameters.declare('method', default='pnorm')
        ...
        self.parameters.declare('output_max_vM_name', default='max_vM_stress')
    ...
    def init_parameters(self):
        ...
        self.output_max_vM_name = self.parameters['output_max_vM_name']
        ...
        self.op = MaxvMStressOperation(
                  ...
                  output_max_vM_name=self.output_max_vM_name)
    ...
    def define(self):
        ...
        max_vm_stress = csdl.custom(*input_list, op=self.op)
        self.register_output(self.op.output_max_vM_name, max_vm_stress)
...
class MaxvMStressOperation(CustomExplicitOperation):
    ...
    def define(self):
        ...
        self.declare_derivatives(self.output_max_vM_name,
                                 self.input_u_name)
        if self.opt_shape:
            for i, field in enumerate(self.opt_field):
    ...
    def compute_derivatives(self, inputs, derivatives):
        ...
        derivatives[self.output_max_vM_name, self.input_u_name] = dmax_vmdu_IGA

        if self.opt_shape:
            for i, field in enumerate(self.opt_field):
                dmax_vmdcp_IGA = self.max_vm_exop.dmax_vMdCPIGA_global(
                                 field, array=True)
                derivatives[self.output_max_vM_name, 
                         self.input_cp_iga_name_list[i]] = dmax_vmdcp_IGA
        if self.opt_thickness:
            dmax_vmdh_th = self.max_vm_exop.dmax_vMdh_th_global(array=True)
            derivatives[self.output_max_vM_name, self.input_h_th_name] = \
                dmax_vmdh_th
