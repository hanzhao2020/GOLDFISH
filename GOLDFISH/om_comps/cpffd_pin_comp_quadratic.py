...
class CPFFDPinCompQuad(om.ExplicitComponent):
    ...
    def init_parameters(self):
        ...
        # self.pin_dof = self.nonmatching_opt_ffd.shopt_pin_dof
        self.opt_field = self.nonmatching_opt_ffd.opt_field

        # self.output_shape = self.nonmatching_opt_ffd.shopt_cppin_size
        # self.deriv = self.nonmatching_opt_ffd.shopt_dcppindcpffd

        self.init_cpffd = []
        if self.nonmatching_opt_ffd.shopt_multiffd:
            self.input_shape = np.sum(self.nonmatching_opt_ffd.shopt_cpffd_size_list)
            self.pin_dofs = self.nonmatching_opt_ffd.shopt_pin_dof_multiffd
            self.derivs = self.nonmatching_opt_ffd.shopt_dcppindcpmultiffd
            self.output_shapes = []
            ...
            for i, field in enumerate(self.opt_field):
                ...
                self.output_shape = self.nonmatching_opt_ffd.shopt_cppin_size
            self.derivs = [self.deriv for i in range(len(self.opt_field))]
            self.output_shapes = [self.output_shape for i in range(len(self.opt_field))]
    ...
    def set_pin_constraint_vals(self, pin_cons_val):
        self.pin_cons_val = pin_cons_val

    def setup(self):
        for i, field in enumerate(self.opt_field):
            self.add_input(self.input_cpffd_name_list[i],
                            shape=self.input_shape,
                            val=self.init_cpffd[i]+np.random.random(self.input_shape))
    ...
    def compute(self, inputs, outputs):
        for i, field in enumerate(self.opt_field):
            deriv_row = self.derivs[i].row
            deriv_col = self.derivs[i].col
            output_val = 0
            input_array = inputs[self.input_cpffd_name_list[i]]
            for pin_ind in range(self.derivs[i].shape[0]):
                dof0 = deriv_col[pin_ind]
                output_val += (input_array[dof0] 
                                - self.pin_cons_val[i][pin_ind])**2
            outputs[self.output_cppin_name_list[i]] = output_val
    ...
    def compute_partials(self, inputs, partials):
        for i, field in enumerate(self.opt_field):
            ...
            for pin_ind in range(self.derivs[i].shape[0]):
                dof0 = deriv_col[pin_ind]
                deriv_array[dof0] = 2*(input_array[dof0] 
                                    - self.pin_cons_val[i][pin_ind])
            partials[self.output_cppin_name_list[i], 
...
if __name__ == "__main__":
    ...
    # prob = Problem()
    # comp = CPFFDPinCompQuad(nonmatching_opt_ffd=nonmatching_opt)
    ...
    comp.init_parameters()
    cpffd_pin_cons_val =  []
    for i, field in enumerate(comp.opt_field):
        cpffd_pin_cons_val += [comp.init_cpffd[i][comp.pin_dofs[i]]]
    comp.set_pin_constraint_vals(cpffd_pin_cons_val)
    prob.model = comp
    prob.setup()
    prob.run_model()
    prob.model.list_outputs()
