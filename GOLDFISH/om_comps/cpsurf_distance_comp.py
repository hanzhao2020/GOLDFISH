...
class CPSurfDistanceComp(om.ExplicitComponent):

    def initialize(self):
        self.options.declare('cpdesign2analysis')
        self.options.declare('input_cp_design_name_pre', default='CP_design')
        self.options.declare('output_cp_dist_name_pre', default='CP_regu')
    ...
    def init_parameters(self):
        ...
        # self.dist_field = []
        # for i, mat in enumerate(self.cpdesign2analysis.cp_coarse_dist_deriv_list):
        #     if mat is not None:
        #         self.dist_field += [self.opt_field[i]]

        self.dist_field = self.cpdesign2analysis.cp_coarse_dist_field
        self.derivs = [mat for mat in self.cpdesign2analysis.cp_coarse_dist_deriv_list 
                       if mat is not None]
        ...
        self.init_cp_design = []
        for i, field in enumerate(self.dist_field):
        ...
        self.output_cp_dist_name_list = []
        for i, field in enumerate(self.dist_field):
            self.input_cp_design_name_list += \
                [self.input_cp_design_name_pre+str(field)]
            self.output_cp_dist_name_list += \
                [self.output_cp_dist_name_pre+str(field)]
    ...
    def setup(self):
        for i, field in enumerate(self.dist_field):
            ...
            self.add_output(self.output_cp_dist_name_list[i],
                            shape=self.output_shapes[i])
            self.declare_partials(self.output_cp_dist_name_list[i],
    ...
    def compute(self, inputs, outputs):
        for i, field in enumerate(self.dist_field):
            outputs[self.output_cp_dist_name_list[i]] = \
...
if __name__ == "__main__":
    ...
    prob = Problem()
    comp = CPSurfDistanceComp(cpdesign2analysis=cpsurfd2a)
