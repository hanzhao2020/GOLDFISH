...
class CPSurfPinComp(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare('output_cp_pin_name_pre', default='CP_regu')
    ...
    def init_parameters(self):
        ...
        # self.pin_field = []
        # for i, mat in enumerate(self.cpdesign2analysis.cp_coarse_pin_deriv_list):
        #     if mat is not None:
        #         self.pin_field += [self.opt_field[i]]
        self.pin_field = self.cpdesign2analysis.cp_coarse_pin_field
        self.derivs = [mat for mat in self.cpdesign2analysis.cp_coarse_pin_deriv_list 
                       if mat is not None]
        ...
        self.output_cp_pin_name_list = []
        for i, field in enumerate(self.pin_field):
            self.input_cp_design_name_list += \
                [self.input_cp_design_name_pre+str(field)]
            self.output_cp_pin_name_list += \
                [self.output_cp_pin_name_pre+str(field)]
    ...
    def setup(self):
        for i, field in enumerate(self.pin_field):
            ...
            self.add_output(self.output_cp_pin_name_list[i],
                            shape=self.output_shapes[i])
            self.declare_partials(self.output_cp_pin_name_list[i],
    ...
    def compute(self, inputs, outputs):
        for i, field in enumerate(self.pin_field):
            outputs[self.output_cp_pin_name_list[i]] = \
                self.derivs[i]*inputs[self.input_cp_design_name_list[i]] \
                - np.array(self.cpdesign2analysis.cp_coarse_pin_vals[field])
...
if __name__ == "__main__":
    ...
    prob = Problem()
    comp = CPSurfPinComp(cpdesign2analysis=cpsurfd2a)
