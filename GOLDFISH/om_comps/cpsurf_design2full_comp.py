...
class CPSurfDesign2FullComp(om.ExplicitComponent):
    def initialize(self):
        self.options.declare('nonmatching_opt')
        self.options.declare('input_cp_design_name', default='CP_Design')
        self.options.declare('output_cp_iga_name_pre', default='CP_IGA')

    def init_parameters(self):
        self.nonmatching_opt = self.options['nonmatching_opt']
        # self.pin_surf_inds = self.options['pin_surf_inds']
        self.input_cp_design_name = self.options['input_cp_design_name']
        ...
        for field_ind, field in enumerate(self.opt_field):
            ...
            self.output_shapes += [len(self.nonmatching_opt.cpdes_iga_dofs_full[field_ind])]

        self.partial_mat = self.nonmatching_opt.shopt_dcpaligndcpsurf

        self.input_CP_design_name_list = []
        self.output_CPS_name_list = []
        for i, field in enumerate(self.opt_field):
            self.input_CP_design_name_list += [self.input_cp_design_name+str(field),]
            self.output_CPS_name_list += [self.output_cp_iga_name_pre+str(field)]
    ...
    def setup(self):
        for i, field in enumerate(self.opt_field):
            ...
            self.add_output(self.output_CPS_name_list[i], 
                            shape=self.output_shapes[i])
            self.declare_partials(self.output_CPS_name_list[i],
    ...
    def compute(self, inputs, outputs):
        for i, field in enumerate(self.opt_field):
            outputs[self.output_CPS_name_list[i]] = \
                self.partial_mat[i]*inputs[self.input_CP_design_name_list[i]]
...
if __name__ == '__main__':
    ...
    prob = Problem()
    comp = CPSurfDesign2FullComp(nonmatching_opt=nonmatching_opt_ffd)
