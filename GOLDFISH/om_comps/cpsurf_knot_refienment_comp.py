...
class CPSurfKnotRefinementComp(om.ExplicitComponent):

    def initialize(self):
        self.options.declare('cpdesign2analysis')
        self.options.declare('input_cp_order_ele_name_pre', default='CP_order_ele')
        self.options.declare('output_cp_fine_name_pre', default='CP_fine')

    def init_parameters(self):
        self.cpdesign2analysis = self.options['cpdesign2analysis']
        self.input_cp_order_ele_name_pre = self.options['input_cp_order_ele_name_pre']
        self.output_cp_fine_name_pre = self.options['output_cp_fine_name_pre']

        self.opt_field = self.cpdesign2analysis.opt_field
        self.derivs = self.cpdesign2analysis.knot_refine_operator_list

        self.input_shapes = [mat.shape[1] for mat in self.derivs]
        self.output_shapes = [mat.shape[0] for mat in self.derivs]

        self.input_cp_order_ele_name_list = []
        self.output_cp_fine_name_list = []
        for i, field in enumerate(self.opt_field):
            self.input_cp_order_ele_name_list += \
                [self.input_cp_order_ele_name_pre+str(field)]
            self.output_cp_fine_name_list += \
                [self.output_cp_fine_name_pre+str(field)]

    def setup(self):
        for i, field in enumerate(self.opt_field):
            self.add_input(self.input_cp_order_ele_name_list[i],
                           shape=self.input_shapes[i])
            self.add_output(self.output_cp_fine_name_list[i],
                            shape=self.output_shapes[i])
            self.declare_partials(self.output_cp_fine_name_list[i],
                                  self.input_cp_order_ele_name_list[i],
    ...
    def compute(self, inputs, outputs):
        for i, field in enumerate(self.opt_field):
            outputs[self.output_cp_fine_name_list[i]] = \
                self.derivs[i]*inputs[self.input_cp_order_ele_name_list[i]]
...
if __name__ == "__main__":
    ...
    prob = Problem()
    comp = CPSurfKnotRefinementComp(cpdesign2analysis=cpsurfd2a)
