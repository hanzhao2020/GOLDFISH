...
class CPFFDPinComp(om.ExplicitComponent):
    ...
    def init_parameters(self):
        ...
        self.pin_field = self.nonmatching_opt_ffd.pin_field
        # self.output_shape = self.nonmatching_opt_ffd.shopt_cppin_size
        # self.derivs = self.nonmatching_opt_ffd.shopt_dcppindcpffd

        if self.nonmatching_opt_ffd.shopt_multiffd:
            self.init_cpffd = self.nonmatching_opt_ffd.shopt_init_cp_mffd_design
            self.derivs = self.nonmatching_opt_ffd.shopt_dcppindcp_mffd
            self.input_shapes = []
            self.output_shapes = []
            for field in self.pin_field:
                field_ind = self.opt_field.index(field)
                self.input_shapes += [self.derivs[field_ind].shape[1]]
                self.output_shapes += [self.derivs[field_ind].shape[0]]
        else:
        ...
        for i, field in enumerate(self.pin_field):
            self.input_cpffd_name_list += \
    ...
    def setup(self):
        for i, field in enumerate(self.pin_field):
            ...
            self.declare_partials(self.output_cppin_name_list[i],
                                ...
                                val=self.derivs[field_ind].data,
                                rows=self.derivs[field_ind].row,
                                cols=self.derivs[field_ind].col)

    def compute(self, inputs, outputs):
        for i, field in enumerate(self.pin_field):
            field_ind = self.opt_field.index(field)
            outputs[self.output_cppin_name_list[i]] = \
                self.derivs[field_ind]*inputs[self.input_cpffd_name_list[i]]
