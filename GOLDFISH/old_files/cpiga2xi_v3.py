...
class CPIGA2Xi(object):
    ...
    # def __init__(self, preprocessor, diff_int_inds=None, opt_field=[0,1,2]):
        ...
        self.get_diff_intersections_edge_cons_info()
    ...
    def get_diff_intersections_edge_cons_info(self):
        ...
        """
        self.int_edge_cons_local_dofs_list = [np.zeros(0, dtype='int32') for i in self.diff_int_inds]
        self.int_edge_cons_dofs_list = [np.zeros(0, dtype='int32') for i in self.diff_int_inds]
        self.int_edge_cons_vals_list = [np.zeros(0, dtype='int32') for i in self.diff_int_inds]
        for i, diff_int_ind in enumerate(self.diff_int_inds):
            ...
            if int_type[0] == 'surf-edge' or int_type[0] == 'edge-surf':
                ...
                self.int_edge_cons_dofs_list[i] = cons_dofs_temp
                self.int_edge_cons_vals_list[i] = cons_vals_temp
                self.int_edge_cons_local_dofs_list[i] = cons_local_dofs_temp
    ...
    def solve_xi(self, xi_flat_init, rtol=1e-3, max_iter=200):
        ...
        while True:
            ...
            if iter_ind > max_iter:
                print(f"Max number of iterations {max_iter} exceeded, rel err: {rel_r_norm}, true_err: {r_norm}")
                max_ind = np.argmax(r)
                print(f"max_ind err: {max_ind}, max err val: {r[max_ind]}")
                break
            if rel_r_norm < rtol:
                print(f"Newton solver converged with {iter_ind} iterations, rel err: {rel_r_norm}, , true_err: {r_norm}")
                break

            self.drdxi = self.dRdxi(xi_root, coo=False)
            self.b_vec = -r.copy()

            self.drdxi_init = self.drdxi.copy()

            for edge_ind, edge_dof in enumerate(self.int_edge_cons_dofs):
                # self.drdxi[edge_ind*3+2,0:edge_dof] = 0
                # self.drdxi[edge_ind*3+2,edge_dof+2:] = 0
                ind_off = 2
                self.drdxi[edge_ind*3+ind_off,:] = 0
                # self.drdxi[:,edge_dof-26] = np.zeros((xi_root.size,1))
                self.drdxi[edge_ind*3+ind_off, edge_dof] = 1.
                self.b_vec[edge_dof] = 0.

            dxi = np.linalg.solve(self.drdxi, self.b_vec)
            xi_root += dxi
            iter_ind += 1
        print("xi_root:", xi_root)
        return xi_root
    ...
    def dRdxi_sub(self, int_ind, xi_flat_sub, coo=True):
        ...
        #     edge_val = self.int_edge_cons_vals_list[int_ind]
        #     for i in range(num_pts):
        #         deriv_xi[i+num_pts*3, edge_local_dof[i]] = self.int_edge_pen
