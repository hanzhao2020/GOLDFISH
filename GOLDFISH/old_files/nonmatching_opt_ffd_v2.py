...
class NonMatchingOptFFD(NonMatchingOpt):
    ...
    def set_shopt_pin_CP_multiFFD(self, ffd_ind, pin_dir0, pin_side0, pin_dir1=None, pin_side1=None):
    ...
    def set_shopt_regu_CPFFDOld(self, shopt_regu_dir, shopt_regu_side, shopt_regu_align=None):
        """
        ...
        should be the direction from ``opt_field``.
        ...
        self.shopt_dcpregudcpffd_list = self.dCPregudCPFFD(
            ...
            self.shopt_regu_side, self.shopt_regu_align, self.opt_field)
    ...
    def set_shopt_regu_CP_multiFFD_old(self, shopt_regu_dir_list, shopt_regu_side_list, shopt_regu_align_list=None):
        assert len(shopt_regu_dir_list) == self.shopt_num_ffd
        self.shopt_regu_dir_list = shopt_regu_dir_list
        self.shopt_regu_side_list = shopt_regu_side_list
        self.shopt_regu_align_list = shopt_regu_align_list

        self.shopt_cpregu_sizes_multiffd_list = [None for ffd_ind in range(self.shopt_num_ffd)]
        self.shopt_dcpregudcpmultiffd_list = [None for ffd_ind in range(self.shopt_num_ffd)]

        for ffd_ind in range(self.shopt_num_ffd):
            shopt_regu_dir = self.shopt_regu_dir_list[ffd_ind]
            shopt_regu_side = self.shopt_regu_side_list[ffd_ind]
            if self.shopt_regu_align_list is not None:
                shopt_regu_align = self.shopt_regu_align_list[ffd_ind]
            else:
                shopt_regu_align = None

            shopt_cpregu_sizes_sub = []
            for i, field in enumerate(self.opt_field):
                cpregu_size_temp = 1
                for j in range(self.nsd):
                    if j == field:
                        cpregu_size_temp *= self.shopt_cpffd_shape_list[ffd_ind][j]-1
                    else:
                        cpregu_size_temp *= self.shopt_cpffd_shape_list[ffd_ind][j]
                if shopt_regu_dir[i] is not None:
                    cpregu_size_temp /= self.shopt_cpffd_shape_list[ffd_ind]\
                                        [shopt_regu_dir[i]]
                if shopt_regu_align is not None:
                    cpregu_size_temp /= self.shopt_cpffd_shape_list[ffd_ind]\
                                        [shopt_regu_align[i]]
                shopt_cpregu_sizes_sub += [int(cpregu_size_temp),]

            shopt_dcpregudcpffd_sub = self.dCPregudCPFFD(
                shopt_cpregu_sizes_sub, self.shopt_cpffd_size_list[ffd_ind], 
                self.shopt_cpffd_shape_list[ffd_ind], shopt_regu_dir, 
                shopt_regu_side, shopt_regu_align, self.opt_field)

            self.shopt_cpregu_sizes_multiffd_list[ffd_ind] = shopt_cpregu_sizes_sub
            self.shopt_dcpregudcpmultiffd_list[ffd_ind] = shopt_dcpregudcpffd_sub       

        self.shopt_cpregu_sizes_multiffd = [None for field in self.opt_field]
        self.shopt_dcpregudcpmultiffd = [None for field in self.opt_field]
        for i, field in enumerate(self.opt_field):
            cpregu_sizes_temp_list = [self.shopt_cpregu_sizes_multiffd_list[ffd_ind][i] 
                                      for ffd_ind in range(self.shopt_num_ffd)]
            self.shopt_cpregu_sizes_multiffd[i] = np.sum(cpregu_sizes_temp_list)
            dcpregudcpffd_temp_list = [self.shopt_dcpregudcpmultiffd_list[ffd_ind][i]
                                       for ffd_ind in range(self.shopt_num_ffd)]
            self.shopt_dcpregudcpmultiffd[i] = block_diag(dcpregudcpffd_temp_list)

        return self.shopt_dcpregudcpmultiffd
    ...
    def set_thopt_regu_CPFFD(self, thopt_regu_dir, thopt_regu_side, thopt_regu_align=None):
        """
        ``shopt_regu_dir`` is a list that has the same length with 
        ``opt_field``. If the entry is None, that means regularize all 
        layers minus one of control points along the direction in 
    ...
    # def dCPpindCPFFDOld(self, cpffd_size, pin_dof):
    ...
    def dCPregudCPFFD_v1(self, cpregu_sizes, cpffd_size, cpffd_shape, regu_dir, regu_side, align_dir=None, opt_field=None):
    ...
    def dCPregudCPFFD_v0(self, cpregu_sizes, cpffd_size, cpffd_shape, regu_dir, regu_side):
        derivs = [np.zeros((cpregu_sizes[i], cpffd_size)) 
    ...
    '''
    def dCPaligndCPFFDold(self, align_dir, cp_align_size, cpffd_size, cpffd_shape):
