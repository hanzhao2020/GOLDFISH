...
class CPFE2IGAImOperation(object):
    ...
    def __init__(self, nonmatching_opt):
        ...
        for i, field in enumerate(self.opt_field):
            self.cp_fe_vecs += [self.nonmatching_opt.
                                vec_scalar_fe_nest.copy()]
            self.cp_iga_vecs += [self.nonmatching_opt.
                                 vec_scalar_iga_nest.copy()]

        self.dRdcp_iga_list = []
        self.dRdcp_iga_csr_list = []
        self.dRdcp_fe_list = []
        self.dRdcp_fe_csr_list = []
        for s_ind in range(self.num_splines):
            ...
            McT.transpose()
            dRdcp_fe_sub = -McT
            self.dRdcp_iga_list += [dRdcp_iga_sub]
            self.dRdcp_iga_csr_list += [csr_matrix(dRdcp_iga_sub.
                                        getValuesCSR()[::-1],
                                        shape=dRdcp_iga_sub.size)]
            self.dRdcp_fe_list += [dRdcp_fe_sub]
            self.dRdcp_fe_csr_list += [csr_matrix(dRdcp_fe_sub.
                                       getValuesCSR()[::-1],
                                       shape=dRdcp_fe_sub.size)]

        dRdcp_iga_dense_list = [csr_mat.todense() for csr_mat
                                in self.dRdcp_iga_csr_list]
        dRdcp_fe_dense_list = [csr_mat.todense() for csr_mat
                               in self.dRdcp_fe_csr_list]
        self.dRdcp_iga_coo = coo_matrix(block_diag(*dRdcp_iga_dense_list))
        self.dRdcp_fe_coo = coo_matrix(block_diag(*dRdcp_fe_dense_list))
        ...
        self.dcp_iga_vec = self.nonmatching_opt.vec_scalar_iga_nest.copy()
        self.dcp_fe_vec = self.nonmatching_opt.vec_scalar_fe_nest.copy()
    ...
    def apply_nonlinear(self):
        ...
        for i, field in enumerate(self.opt_field):
            ...
            for s_ind in range(self.num_splines):
                Mc = m2p(self.nonmatching_opt.splines[s_ind].M_control)
                cp_iga_sub = self.cp_iga_vecs[i].getNestSubVecs()[s_ind]
                cp_fe_sub = self.cp_fe_vecs[i].getNestSubVecs()[s_ind]
    ...
    def solve_nonlinear(self):
        ...
        for i, field in enumerate(self.opt_field):
            ...
            for s_ind in range(self.num_splines):
                ...
                cp_iga_sub = self.cp_iga_vecs[i].getNestSubVecs()[s_ind]
                cp_fe_sub = self.cp_fe_vecs[i].getNestSubVecs()[s_ind]
                McTMc = self.dRdcp_iga_list[s_ind]
    ...
    def apply_linear_fwd(self, d_inputs_array_list=None, 
        ...
        if d_residuals_array_list is not None:
            if d_outputs_array_list is not None:
                for i, field in enumerate(self.opt_field):
                    update_nest_vec(d_outputs_array_list[i], self.dcp_iga_vec)
                    dres_array_out_list = []
                    for s_ind in range(self.num_splines):
                        cp_iga_sub = self.dcp_iga_vec.getNestSubVecs()[s_ind]
                        dres_sub_out = A_x(self.dRdcp_iga_list[s_ind], 
                                           cp_iga_sub)
            ...
            if d_inputs_array_list is not None:
                for i, field in enumerate(self.opt_field):
                    update_nest_vec(d_inputs_array_list[i], self.dcp_fe_vec)
                    dres_array_in_list = []
                    for s_ind in range(self.num_splines):
                        cp_fe_sub = self.dcp_fe_vec.getNestSubVecs()[s_ind]
                        dres_sub_in = A_x(self.dRdcp_fe_list[s_ind], 
                                          cp_fe_sub)
    ...
    def apply_linear_rev(self, d_inputs_array_list=None, 
        ...
        if d_residuals_array_list is not None:
            for i, field in enumerate(self.opt_field):
                update_nest_vec(d_residuals_array_list[i], self.dres_vec)
            if d_outputs_array_list is not None:
                for i, field in enumerate(self.opt_field):
                    dcp_iga_array_list = []
                    for s_ind in range(self.num_splines):
                        dres_sub = self.dres_vec.getNestSubVecs()[s_ind]
                        dcp_iga_sub = self.dcp_iga_vec.getNestSubVecs()[s_ind]
                        AT_x_b(self.dRdcp_iga_list[s_ind], dres_sub, 
                               dcp_iga_sub)
            ...
            if d_inputs_array_list is not None:
                for i, field in enumerate(self.opt_field):
                    ...
                    for s_ind in range(self.num_splines):
                        dres_sub = self.dres_vec.getNestSubVecs()[s_ind]
                        dcp_fe_sub = self.dcp_fe_vec.getNestSubVecs()[s_ind]
                        AT_x_b(self.dRdcp_fe_list[s_ind], dres_sub, 
                               dcp_fe_sub)
        ...
        for i, field in enumerate(self.opt_field):
            ...
            for s_ind in range(self.num_splines):
                ...
                dres_sub.setValues(range(dres_sub.sizes[1]),
                                    dres_sub_array)
                dres_sub.assemble()
                solve(PETScMatrix(self.dRdcp_iga_list[s_ind]),
