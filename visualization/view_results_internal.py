...
file_path_local = file_path_mac_local
# file_path_local = file_path_ubuntu_local
...
# transform_ind1 = np.arange(0,72,4)
transform_ind1 = []
translate_val1 = [0.,0.,1.]

# transform_ind2 = np.arange(1,72,4)
transform_ind2 = []
translate_val2 = [0.,0.,-1.]

# transform_ind3 = np.arange(2,72,4)
transform_ind3 = []
translate_val3 = [-1.,0.,0.]

# transform_ind4 = np.arange(3,72,4)
transform_ind4 = []
translate_val4 = [1.,0.,0.]

parser = argparse.ArgumentParser()

parser.add_argument('--res_ind', dest='res_ind',default=2,
                    help="File index.")

# parser.add_argument('--file_path', dest='file_path', #default='./',
#                     # default=file_path_mac_local+"results"+str(result_ind)+"/",
#                     default=file_path_local+"results"+str(result_ind)+"/",
...
parser.add_argument('--geom_opacity', dest='geom_opacity', default=0.9,
...
parser.add_argument('--disp_opacity', dest='disp_opacity', default=0.9,
...
result_ind = int(args.res_ind)

file_path = file_path_local+"results"+str(result_ind)+"/"
...
print("Results index:", result_ind)
...
# for surf_ind in custom_inds:
    ...
    if show_geom:
        if show_transform:
            if surf_ind not in transform_ind1 and surf_ind not in transform_ind2 \
            and surf_ind not in transform_ind3 and surf_ind not in transform_ind4:
                ...
                if show_geom_edge:
                    ...
                    warpByVector1Display.LineWidth = 3.0
        else:
            warpByVector1Display = Show(warpByVector1, renderView1, 
                                            'UnstructuredGridRepresentation')
            ColorBy(warpByVector1Display, None)
    ...
    # Transform cetertain patches
    if show_transform:
        if surf_ind in transform_ind1:
            transform1 = Transform(registrationName='Transform'+str(surf_ind), Input=warpByVector1)
            transform1.Transform = 'Transform'
            # Properties modified on transform1.Transform
            transform1.Transform.Translate = translate_val1
            transform1Display = Show(transform1, renderView1, 'UnstructuredGridRepresentation')
            # ColorBy(warpByVector2Display, ('POINTS', 'Result', 'Magnitude'))
            ColorBy(transform1Display, None)
            transform1Display.Opacity = geom_opacity
            if show_geom_edge:
                transform1Display.SetRepresentationType('Surface With Edges')
                transform1Display.LineWidth = 3.0

        elif surf_ind in transform_ind2:
            transform1 = Transform(registrationName='Transform1'+str(surf_ind), Input=warpByVector1)
            transform1.Transform = 'Transform'
            # Properties modified on transform1.Transform
            transform1.Transform.Translate = translate_val2
            transform1Display = Show(transform1, renderView1, 'UnstructuredGridRepresentation')
            # ColorBy(warpByVector2Display, ('POINTS', 'Result', 'Magnitude'))
            ColorBy(transform1Display, None)
            transform1Display.Opacity = geom_opacity
        ...
        elif surf_ind in transform_ind3:
            transform1 = Transform(registrationName='Transform1'+str(surf_ind), Input=warpByVector1)
            transform1.Transform = 'Transform'
            # Properties modified on transform1.Transform
            transform1.Transform.Translate = translate_val3
            transform1Display = Show(transform1, renderView1, 'UnstructuredGridRepresentation')
            # ColorBy(warpByVector2Display, ('POINTS', 'Result', 'Magnitude'))
            ColorBy(transform1Display, None)
            transform1Display.Opacity = geom_opacity
        ...
        elif surf_ind in transform_ind4:
            transform1 = Transform(registrationName='Transform1'+str(surf_ind), Input=warpByVector1)
            transform1.Transform = 'Transform'
            # Properties modified on transform1.Transform
            transform1.Transform.Translate = translate_val4
            transform1Display = Show(transform1, renderView1, 'UnstructuredGridRepresentation')
            # ColorBy(warpByVector2Display, ('POINTS', 'Result', 'Magnitude'))
            ColorBy(transform1Display, None)
            transform1Display.Opacity = geom_opacity
...
# ##########################################################
# # Show eVTOL wing geometry
# for i in range(end_ind):
#     # create a new 'Legacy VTK Reader'
#     surf0vtk = LegacyVTKReader(registrationName='surf'+str(i)+'.vtk', 
#                FileNames=['/home/han/OneDrive/github/GOLDFISH/demos_om/shape_opt/eVTOL/geometry/surf_geom/surf'+str(i)+'.vtk'])

#     # get active view
#     renderView1 = GetActiveViewOrCreate('RenderView')

#     # show data in view
#     surf0vtkDisplay = Show(surf0vtk, renderView1, 'StructuredGridRepresentation')
#     if i > 3:
#         surf0vtkDisplay.Opacity = 0.2
#     else:
#         surf0vtkDisplay.Opacity = 0.3
# ##########################################################
