...
class SplineSim(object):
    ...
    def dRIGAdxi_sub(self, index, side=0):
        """
...
if __name__ == "__main__":
    ...
    print("rel_err off-diag:", rel_err)

    time0 = perf_counter()
    dRIGAdxi_sub = spline_sim.dRIGAdxi_sub(index=index, side=side)
    time1 = perf_counter()
    dRIGAdxi_diag_FD = spline_sim.dRIGAdxi_FD_diag(index=index, side=side)
    dRIGAdxi_offdiag_FD = spline_sim.dRIGAdxi_FD_offdiag(index=index, side=side)
    time2 = perf_counter()

    if side == 0:
        diff0 = dRIGAdxi_sub[0][:,:] - dRIGAdxi_diag_FD
        diff1 = dRIGAdxi_sub[1][:,:] - dRIGAdxi_offdiag_FD
    if side == 1:
        diff0 = dRIGAdxi_sub[1][:,:] - dRIGAdxi_diag_FD
        diff1 = dRIGAdxi_sub[0][:,:] - dRIGAdxi_offdiag_FD
    err0 = np.linalg.norm(diff0)
    err1 = np.linalg.norm(diff1)
    print("Relative error diag:", err0/np.linalg.norm(dRIGAdxi_diag_FD))
    print("Relative error off-diag:", err1/np.linalg.norm(dRIGAdxi_offdiag_FD))

    time_ana = time1 - time0
    time_fd = time2 - time1

    print("Analytical derivative time: {:8.6f}, FD derivative time: {:8.6f}"
