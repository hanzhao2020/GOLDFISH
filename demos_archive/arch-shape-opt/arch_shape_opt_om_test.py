...
class ShapeOptGroup(om.Group):
    ...
    def setup(self):
        ...
        for i, field in enumerate(self.opt_field):
            ...
            # self.add_constraint(self.disp_states_comp_name+'.'
            #                     +self.disp_name,
            #                     upper=1e-2)
...
# Save non-matching intersections coordinates
# phy_loc = []
# nonmatching_interface = []
# for i in range(len(preprocessor.intersections_phy_coords)):
#     # intersections_phy_coords_interp = generate_interpolated_data(preprocessor.intersections_phy_coords[i], 30)
#     # phy_loc = interface_physical_location(spline_sim.nonmatching.splines[preprocessor.mapping_list[i][0]], intersections_phy_coords_interp)
#     phy_loc = generate_interpolated_data(preprocessor.intersections_phy_coords[i], 60)
#     mesh = generate_mortar_mesh(data=phy_loc)
#     nonmatching_interface += [mesh,]
#     file_name = save_path + "results"+str(test_ind)+"/" + "interface_"+str(i)+".pvd"
#     File(file_name) << mesh