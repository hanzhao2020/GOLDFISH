...
class ThicknessOptGroup(om.Group):
    ...
    def setup(self):
        ...
        rho = 1e2
        upper_vM = 1e6
        self.max_vM_comp = MaxvMSressComp(
...
for i in range(nonmatching_opt.num_splines):
    spline_stress = ShellStressSVK(nonmatching_opt.splines[i], 
                                   ...
                                   E, nu, nonmatching_opt.h_th[i], linearize=True,) 
    von_Mises_top = spline_stress.vonMisesStress(nonmatching_opt.h_th[i]/2)
    von_Mises_top_proj = nonmatching_opt.splines[i].projectScalarOntoLinears(
                         von_Mises_top, lumpMass=False)
    # VvM = FunctionSpace(nonmatching_opt.splines[i].mesh, 'CG', 2)
    # von_Mises_top_proj = spline_project(nonmatching_opt.splines[i], VvM,
    #                      von_Mises_top)
    von_Mises_tops += [von_Mises_top_proj]
    vM_max_list += [v2p(von_Mises_tops[i].vector()).max()[1]]
