...
from IGAOPT.nonmatching_opt_om import *
...
class ShapeOptGroup(om.Group):
    ...
    def initialize(self):
        ...
        # self.options.declare('cpffd_align_dir', default=1)
        # self.options.declare('cpffd_pin_dir0', default=2)
        # self.options.declare('cpffd_pin_side0', default=[0])
        # self.options.declare('cpffd_pin_dir1', default=1)
        # self.options.declare('cpffd_pin_side1', default=[0])
        ...
        # self.options.declare('compliance_name', default='compliance')
    ...
    def init_parameters(self):
        ...
        # self.cpffd_align_dir = self.options['cpffd_align_dir']
        # self.cpffd_pin_dir0 = self.options['cpffd_pin_dir0']
        # self.cpffd_pin_side0 = self.options['cpffd_pin_side0']
        # self.cpffd_pin_dir1 = self.options['cpffd_pin_dir1']
        # self.cpffd_pin_side1 = self.options['cpffd_pin_side1']
    ...
    def setup(self):
        ...
        for i, field in enumerate(self.opt_field):
            # init_cpffd = self.nonmatching_opt_ffd.cpffd_flat[:,field] \
            #            + np.sin(self.nonmatching_opt_ffd.cpffd_flat[:,field])*1e-3
        ...
        # # Add Compliance comp (objective function)
        # self.compliance_comp = ComplianceComp(
        #                   nonmatching_opt=self.nonmatching_opt_ffd,
        #                   input_cp_iga_name_pre=self.cpsurf_iga_name_pre,
        #                   input_u_name=self.disp_name,
        #                   output_c_name=self.compliance_name)
        # self.compliance_comp.init_parameters()
        # self.add_subsystem(self.compliance_comp_name, self.compliance_comp)
        ...
        self.cpffd_pin_comp = CPFFDPinComp(
                         nonmatching_opt_ffd=self.nonmatching_opt_ffd,
                         # pin_dir0=self.cpffd_pin_dir0, 
                         # pin_side0=self.cpffd_pin_side0,
                         # pin_dir1=self.cpffd_pin_dir1, 
                         # pin_side1=self.cpffd_pin_side1,
                         input_cpffd_name_pre=self.cpffd_name_pre,
        ...
        # self.connect(self.disp_states_comp_name+'.'+self.disp_name,
        #              self.compliance_comp_name+'.'+self.disp_name)
        ...
        self.add_objective(self.int_energy_comp_name+'.'
                           ...
                           scaler=1e8)
        # self.add_objective(self.compliance_comp_name+'.'
...
for s_ind in range(nonmatching_opt_ffd.num_splines):
    ...
    A0,A1,A2,deriv_A2,A,B = surfaceGeometry(nonmatching_opt_ffd.splines[s_ind], X)
    a0,a1,a2,deriv_a2,a,b = surfaceGeometry(nonmatching_opt_ffd.splines[s_ind], x)
    pressure_dir = sqrt(det(a)/det(A))*a2        
    normal_pressure = pressure*pressure_dir
    source_terms += [inner(normal_pressure, z)*nonmatching_opt_ffd.splines[s_ind].dx]
...
# ##############################################################
# for i in range(len(nonmatching_opt_ffd.spline_funcs)):
#     nonmatching_opt_ffd.spline_funcs[i].interpolate(Constant((0.,0.,0.)))    
# nonmatching_opt_ffd.update_mortar_funcs()

# nonmatching_opt_ffd.solve_linear_nonmatching_problem()
# A_norm = nonmatching_opt_ffd.A.norm()
# b_norm = nonmatching_opt_ffd.b.norm()

# print("A norm:", A_norm, ", b_norm:", b_norm)

# # nonmatching_opt_ffd.solve_nonlinear_nonmatching_problem()
# print(aaa)
# ##############################################################
...
if optimizer.upper() == 'SNOPT':
    ...
    prob.driver.opt_settings['Summary file'] = \
        './SNOPT_out/SNOPT_summary_'+str(TEST_IND)+'.out'
    prob.driver.opt_settings['Print file'] = \
        './SNOPT_out/SNOPT_print_'+str(TEST_IND)+'.out'
    prob.driver.options['debug_print'] = ['objs']
    prob.driver.options['print_results'] = True
elif optimizer.upper() == 'SLSQP':
    prob.driver = om.ScipyOptimizeDriver()
    prob.driver.options['optimizer'] = 'SLSQP'
    # prob.driver.options['optimizer'] = 'BFGS' # Deesn't work, raise error
    # prob.driver.options['optimizer'] = 'L-BFGS-B' # Deesn't work, raise error
    # prob.driver.options['optimizer'] = 'trust-constr' # Deesn't work, raise index error
    # prob.driver.options['optimizer'] = 'Newton-CG' # Deesn't work, raise error
    # prob.driver.options['optimizer'] = 'COBYLA' # raise error, not handle eq constraint
    # prob.driver.options['optimizer'] = 'Nelder-Mead' # Testing
    # prob.driver.options['optimizer'] = 'Powell' # Testing

    prob.driver.options['tol'] = 1e-8
    prob.driver.options['disp'] = True
    # prob.driver.options['debug_print'] = ['nl_cons', 'ln_cons', 
    #                                       'desvars', 'objs', 'totals']
    prob.driver.options['debug_print'] = ['objs']
...
VTK().write('./geometry/final_FFD_block.vtk', final_FFD_block)
...
# if 0 in opt_field:
#     final_cpffd0 = prob[model.inputs_comp_name+'.'+model.cpffd_name_list[0]]
#     final_cpffd2 = prob[model.inputs_comp_name+'.'+model.cpffd_name_list[1]]
