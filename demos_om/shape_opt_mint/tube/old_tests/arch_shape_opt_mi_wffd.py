...
class ShapeOptGroup(om.Group):
    ...
    def init_parameters(self):
        ...
        self.design_var_lower = [0., -1.e-3] #z-lower
        self.design_var_upper = [10., 10.] #z-upper
...
filename_igs = "./geometry/init_arch_geom_3patch.igs"
...
spline_bcs = [spline_bc0, None, spline_bc1]
...
residuals = []
for i in range(num_surfs):
    X = nonmatching_opt.splines[i].F
    A0,A1,A2,_,A,B = surfaceGeometry(nonmatching_opt.splines[i],X)
    ...
    force_list += [force]
    source_terms += [inner(force_list[i], nonmatching_opt.splines[i]\
...
shopt_ffd_num_el = [[3,1,2], [3,1,2], [3,1,2]]
shopt_ffd_p = [2]*num_shopt_ffd
extrude_dir = [None, 2, None]
...
a0 = nonmatching_opt.set_shopt_regu_CP_multiFFD(shopt_regu_dir_list=[[None, None], 
                                           ...
                                           shopt_regu_side_list=[[None, None], 
                                                                 [None, None],
                                                                 [None, None]])
a1 = nonmatching_opt.set_shopt_pin_CP_multiFFD(0, pin_dir0_list=[2, None, 2], 
                                          pin_side0_list=[[0], None, [0]],
                                          pin_dir1_list=None, 
                                          pin_side1_list=None)
a2 = nonmatching_opt.set_shopt_pin_CP_multiFFD(2, pin_dir0_list=[2, None, 2], 
                                          pin_side0_list=[[0], None, [0]],
                                          pin_dir1_list=None, 
                                          pin_side1_list=None)
a3 = nonmatching_opt.set_shopt_align_CP_multiFFD(shopt_align_dir_list=[1,1,1])
...
for i in range(0,num_surfs):
    max_F2 = np.max(nonmatching_opt.splines[i].cpFuncs[2].vector().get_local())
    min_F2 = np.min(nonmatching_opt.splines[i].cpFuncs[2].vector().get_local())
    print("Spline: {:2d}, Max F2: {:8.6f}".format(i, max_F2))
    print("Spline: {:2d}, Min F2: {:8.6f}".format(i, min_F2))