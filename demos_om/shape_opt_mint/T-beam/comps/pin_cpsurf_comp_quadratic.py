...
class CPSurfPinComp(om.ExplicitComponent):
    ...
    def setup(self):
        for i, field in enumerate(self.opt_field):
            self.add_input(self.input_CP_name_list[i], 
                           ...
                           val=self.init_cp_iga[:,field]+np.random.random(self.input_shape))
    ...
    def compute(self, inputs, outputs):
        for i, field in enumerate(self.opt_field):
            outputs[self.output_pin_name_list[i]] = 0
            input_cp = inputs[self.input_CP_name_list[i]]
            for j, pin_dof in enumerate(self.pin_dofs):
                # print("pin_dof:", pin_dof)
                outputs[self.output_pin_name_list[i]] += \
                    self.alpha*(input_cp[pin_dof] - self.pin_val[i][j])**2
            print("field: {}, cp surf pin val: {}".
                  format(field, outputs[self.output_pin_name_list[i]]))

    def compute_partials(self, inputs, partials):
        for i, field in enumerate(self.opt_field):
            input_cp = inputs[self.input_CP_name_list[i]]
            partial_vec = np.zeros(self.input_shape)
            for j, pin_dof in enumerate(self.pin_dofs):
                partial_vec[pin_dof] = self.alpha*2*(input_cp[pin_dof] - self.pin_val[i][j])
            partials[self.output_pin_name_list[i], 
                     self.input_CP_name_list[i]] = partial_vec
