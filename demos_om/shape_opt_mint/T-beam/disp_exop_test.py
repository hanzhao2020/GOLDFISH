...
class DispMintExComp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        # for i, field in enumerate(self.opt_field):
        #     self.declare_partials(self.output_res_name,
        #                           self.input_cp_iga_name_list[i], method='fd', form='forward', step=1e-6)
        # self.declare_partials(self.output_res_name, self.input_xi_name, method='fd', form='forward', step=1e-6)
        # self.declare_partials(self.output_res_name, self.input_u_name, method='fd', form='forward', step=1e-6)
    ...
    def update_inputs(self, inputs):
        ...
        # xi_diff = inputs[self.input_xi_name] - self.init_xi
        # print("input xi diff:", np.linalg.norm(xi_diff))
    ...
    def compute_partials(self, inputs, partials):
        self.update_inputs(inputs)
        print(inputs[self.input_xi_name])
        ...
        print("norm:", np.linalg.norm(partials[self.output_res_name, self.input_xi_name]))
