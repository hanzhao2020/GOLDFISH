...
nonmatching_opt.update_uIGA(uiga_init.array)
init_cp_iga = nonmatching_opt.get_init_CPIGA()
nonmatching_opt.update_CPIGA(init_cp_iga[0], 0)
init_xi = nonmatching_opt.xi_nest.array
nonmatching_opt.update_xi(init_xi)
nonmatching_opt.update_transfer_matrices()
...
# move_mortar_mesh(nonmatching_opt.mortar_meshes[int_ind],
#                  preprocessor.intersections_para_coords[int_ind][side])

a00m = nonmatching_opt.dRIGAdxi_sub(int_ind,side)[int(side)]
a10m = nonmatching_opt.dRIGAdxi_sub(int_ind,side)[int(1-side)]
# a00fd = nonmatching_opt.dRIGAdxi_FD_diag_sub(int_ind,side)
# a10fd = nonmatching_opt.dRIGAdxi_FD_offdiag_sub(int_ind,side)

a00c = csr_matrix(a00m.getValuesCSR()[::-1])
a10c = csr_matrix(a10m.getValuesCSR()[::-1])
a00d = a00c.todense()
a10d = a10c.todense()
...
# rel_err00 = np.linalg.norm(err00)/np.linalg.norm(a00fd)
# rel_err10 = np.linalg.norm(err10)/np.linalg.norm(a10fd)
...
a01m = nonmatching_opt.dRIGAdxi_sub(int_ind,side)[0]#int(side)]
a11m = nonmatching_opt.dRIGAdxi_sub(int_ind,side)[1]#int(1-side)]
# a11fd = nonmatching_opt.dRIGAdxi_FD_diag_sub(int_ind,side)
# a01fd = nonmatching_opt.dRIGAdxi_FD_offdiag_sub(int_ind,side)

a01c = csr_matrix(a01m.getValuesCSR()[::-1])
a11c = csr_matrix(a11m.getValuesCSR()[::-1])
a01d = a01c.todense()
a11d = a11c.todense()
...
# rel_err01 = np.linalg.norm(err01)/np.linalg.norm(a01fd)
# rel_err11 = np.linalg.norm(err11)/np.linalg.norm(a11fd)


A0 = np.concatenate([a00d, a01d], axis=1)
A1 = np.concatenate([a10d, a11d], axis=1)
drdxi_sub = np.concatenate([A0,A1], axis=0)


Aam = nonmatching_opt.dRIGAdxi()
Aam.convert('seqaij')
Aa = csr_matrix(Aam.getValuesCSR()[::-1]).todense()
print("Aa norm:", np.linalg.norm(Aa))


Cfd = nonmatching_opt.dRIGAdxi_FD(init_xi)
err = np.linalg.norm(Aa-Cfd)/np.linalg.norm(Cfd)
print("rel error:", err)


# drigadcp_mat = nonmatching_opt.dRIGAdCPIGA(0)
# drigadcp = csr_matrix(drigadcp_mat.getValuesCSR()[::-1]).todense()
# drigadcp_fd = nonmatching_opt.dRIGAdCPIGA_FD(init_cp_iga[0], 0)
# err1 = np.linalg.norm(drigadcp-drigadcp_fd)/np.linalg.norm(drigadcp_fd)
# print("rel error1:", err1)


# from disp_exop_test import *

# comp = DispMintExComp(nonmatching_opt=nonmatching_opt)
# comp.init_parameters()
# p6 = om.Problem(comp)
# p6.setup()
# p6.run_model()
# a0 = p6.check_partials(compact_print=True)
# a1 = a0['']
# drdxi_dict = a1[('res', 'int_para')]
# drdxi_fd = drdxi_dict['J_fd']
# drdxi_ana = drdxi_dict['J_fwd']
