...
from cpsurf_straight_align_comp_rigidspar import CPSurfRigidAlignComp
    ...
    def setup(self):
        ...
        for vol_ind, surf_ind in enumerate(self.vol_surf_inds):
            self.add_constraint(self.volume_comp_name+str(vol_ind)+'.'+self.volume_name+str(vol_ind),
                                upper=3.0*self.vol_val_list[vol_ind])
                                # equals=self.vol_val)
...
# h_th = [Constant(2.0e-3)]*num_surfs  # Thickness of surfaces, m
