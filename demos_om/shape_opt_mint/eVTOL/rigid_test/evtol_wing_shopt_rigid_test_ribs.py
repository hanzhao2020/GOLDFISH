...
from cpsurf_rigid_align_comp_v1 import CPSurfRigidAlignComp


from create_geom_evtol_ribs import preprocessor
...
class ShapeOptGroup(om.Group):
    ...
    def init_parameters(self):
        ...
        self.design_field = [1,2]

        self.init_cp_design = self.cpdesign2analysis.init_cp_design[1:]
        self.input_cp_shapes = [len(cp) for cp in self.init_cp_design]

        self.design_var_lower = [0.8, 2.7] # x min, z min
        self.design_var_upper = [5.0, 3.7] # x max, z max

        self.cp_design_name_list = []
        for field_ind, field in enumerate(self.design_field):
        ...
        #                        - np.dot(self.cpdesign2analysis.cp_coarse_align_deriv_list[field_ind].todense(),
    ...
    def setup(self):
        ...
        # self.vol_surf_inds = [3,4,5,6,7,8,9,10]
        ...
        if len(self.regu_field) > 0:
            for field_ind, field in enumerate(self.regu_field):
                ...
                design_field_ind = self.design_field.index(field)
                self.connect(self.inputs_comp_name+'.'
                         +self.cp_design_name_list[design_field_ind],
        ...
        self.add_constraint(self.int_xi_edge_comp_name+'.'+self.int_xi_edge_name,
                            equals=self.int_xi_edge_cons_val, )#scaler=1e-1)
                            # upper=1e-3, scaler=1)
...
opt_field = [0,1,2]
# shopt_surf_inds = [[3,4,5,6], [5,6], [3,4,5,6]]
# shopt_surf_inds = [[5], [5], [5]]
# shopt_surf_inds = [[3], [3], [3]]

surf_inds = list(range(3,preprocessor.num_surfs))
shopt_surf_inds = [surf_inds, surf_inds, surf_inds]
...
# from cpiga2xi_exop_test import *
# comp = CPIGA2XiExComp(nonmatching_opt=nonmatching_opt)
...
# p6.setup()
# p6.run_model()
# p6.check_partials(compact_print=True)

# _, uiga = nonmatching_opt.solve_nonlinear_nonmatching_problem(iga_dofs=True)
# nonmatching_opt.update_uIGA(uiga.array)
# nonmatching_opt.update_xi(nonmatching_opt.xi_nest.array)
# nonmatching_opt.update_transfer_matrices()

# print('1')
# drdxim = nonmatching_opt.dRIGAdxi()
# drdxim.convert('seqaij')
# print('2')
# drdxia = csr_matrix(drdxim.getValuesCSR()[::-1]).todense()
# drdxifd = nonmatching_opt.dRIGAdxi_FD(nonmatching_opt.xi_nest.array)

# err = drdxia - drdxifd
# err_norm = np.linalg.norm(err)/np.linalg.norm(drdxifd)
...
# t00 = cpsurfd2a.set_cp_align(field=0, align_dir_list=[[0,1],[0,1],[0,1],[0,1]])
# t01 = cpsurfd2a.set_cp_align(field=1, align_dir_list=[[0,1],[0,1]])

# # t10 = cpsurfd2a.set_cp_pin(field=0, pin_dir0_list=[0,0], 
# #                           pin_side0_list=[[0,1], [0,1]])

# t2 = cpsurfd2a.set_cp_regu(2, regu_dir_list=[1,1,1,1], rev_dir=True)
# t30 = cpsurfd2a.set_cp_dist(0, surf_inds=[3,4])
# t31 = cpsurfd2a.set_cp_dist(1, surf_inds=[5,6])
...
# t00 = cpsurfd2a.set_cp_align(field=0, align_dir_list=[[0,1],[0,1],[0,1]])
t01 = cpsurfd2a.set_cp_align(field=1, align_dir_list=[[0,1]]*len(shopt_surf_inds[1]))#, [0,1]])

# t10 = cpsurfd2a.set_cp_pin(field=0, pin_dir0_list=[0,0], 
#                           pin_side0_list=[[0,1], [0,1]])

# t2 = cpsurfd2a.set_cp_regu(2, regu_dir_list=[1], rev_dir=True)
# t30 = cpsurfd2a.set_cp_dist(0, surf_inds=[3,4])
t31 = cpsurfd2a.set_cp_dist(1, surf_inds=surf_inds)

# cpsurfd2a.cp_coarse_dist_deriv_list[0] = coo_matrix(cpsurfd2a.cp_coarse_dist_deriv_list[0].todense()[:,0:2])
cpsurfd2a.init_cp_design[0] = cpsurfd2a.init_cp_design[0][0:0]
