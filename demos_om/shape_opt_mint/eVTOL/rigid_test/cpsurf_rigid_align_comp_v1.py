...
class CPSurfRigidAlignComp(om.ExplicitComponent):
    ...
    def init_parameters(self):
        ...
        for i, field in enumerate(self.design_field):
            self.input_cp_design_name_list += \
        ...
        self.input_shapes = self.input_shapes[1:]
        self.init_cp_design = self.init_cp_design[1:]
        ...
        self.derivs_rib_dxdy = np.zeros((self.output_shapes[0], 
                                         self.input_shapes[0]))
        self.derivs_rib_dxdy_diff = np.zeros(self.output_shapes[0])

        sub_dxdy = np.array([a0, b0, a0, b0])
        sub_diff_vec = np.array([a1, b1, a1, b1])
        for i in range(self.input_shapes[0]):
    ...
    def setup(self):
        ...
        self.declare_partials(self.output_cp_coarse_name_list[i],
                              self.input_cp_design_name_list[i-1],
        ...
        self.declare_partials(self.output_cp_coarse_name_list[0],
                              self.input_cp_design_name_list[0],
        ...
        for i, field in enumerate(self.opt_field):
            if field == 0:
                outputs[self.output_cp_coarse_name_list[i]] = \
                    self.derivs_rib_dxdy*inputs[self.input_cp_design_name_list[i]] \
                    + self.derivs_rib_dxdy_diff
            else:
                outputs[self.output_cp_coarse_name_list[i]] = \
                    self.derivs[i]*inputs[self.input_cp_design_name_list[i-1]] \
                    + self.diff_vec[i]
