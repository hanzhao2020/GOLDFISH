...
from GOLDFISH.cpiga2xi_coarse import *
...
from cpsurf_straight_align_comp import CPSurfStraightAlignComp

from create_geom_evtol_reduced import preprocessor
...
class ShapeOptGroup(om.Group):
        ...
        for field_ind, field in enumerate(self.opt_field):
            if field == 0:
                self.diff_vec += [np.asarray(self.cpdesign2analysis.init_cp_coarse[field_ind] \
                               - np.dot(self.cpdesign2analysis.cp_coarse_align_deriv_list[field_ind].todense()[:,0:4],
    ...
    def setup(self):
        ...
        self.cp_align_comp = CPSurfStraightAlignComp(
...
opt_field = [0,1,2]
# shopt_surf_inds = [[3,4,5,6,7], [5,6,7], [3,4,5,6,7]]
shopt_surf_inds = [[3,4,5,6], [5,6], [3,4,5,6]]
shopt_surf_inds_explicit = [[3,4,5,6], [5,6], []]
...
# for xi in xi_sizes:
#     if xi > 36:
#         extreme_cons_eval_pts += [8]
#     else:
#         extreme_cons_eval_pts += [4]
...
cpsurfd2a = CPSurfDesign2Analysis(preprocessor, opt_field=opt_field, 
            shopt_surf_inds=shopt_surf_inds, 
            shopt_surf_inds_explicit=shopt_surf_inds_explicit)
...
coarse_p = [3,1]
coarse_p_list = [coarse_p]*preprocessor.num_surfs
coarse_knot = [[0]*(coarse_p[0]+1)+[1]*(coarse_p[0]+1), 
                [0]*(coarse_p[1]+1)+[1]*(coarse_p[1]+1)]
coarse_knots_list = [coarse_knot]*preprocessor.num_surfs

cpsurfd2a.set_init_knots_by_field(init_p_list, init_knots_list)
a1 = cpsurfd2a.set_coarse_knots_all(coarse_p_list, coarse_knots_list)
a2 = cpsurfd2a.design2coarse_refinement_opt(coarse_p)
a3 = cpsurfd2a.get_init_design_cp()


t00 = cpsurfd2a.set_cp_align(field=0, align_dir_list=[1,1,[0,1],[0,1]])#,[0,1]])
t01 = cpsurfd2a.set_cp_align(field=1, align_dir_list=[1,1])#,1])

t30 = cpsurfd2a.set_cp_dist(0, surf_inds=[3,4], rev_dir=False)
t31 = cpsurfd2a.set_cp_dist(1, surf_inds=[5,6], rev_dir=False)

cpsurfd2a.cp_coarse_dist_deriv_list[0] = coo_matrix(cpsurfd2a.cp_coarse_dist_deriv_list[0].todense()[:,0:4])
cpsurfd2a.init_design_cp_simp[0] = cpsurfd2a.init_design_cp_simp[0][0:4]


cpiga2xi_coarse = CPIGA2XiCoarse(preprocessor, cpsurfd2a, shopt_surf_inds_explicit, 
                                opt_field, num_edge_pts=4)
...
# # t2 = cpsurfd2a.set_cp_regu(2, regu_dir_list=[1,1,1,1,1], rev_dir=True)
