...
class IntEnergyReguExOperation(object):
    ...
    def __init__(self, nonmatching_opt):
        ...
        for s_ind in range(self.num_splines):
            ...
            wint += 1e-4*(inner(self.splines[s_ind].grad(self.splines[s_ind].cpFuncs[2])[0],
                                self.splines[s_ind].grad(self.splines[s_ind].cpFuncs[2])[0])
                        + inner(self.splines[s_ind].grad(self.splines[s_ind].cpFuncs[2])[1],
                                self.splines[s_ind].grad(self.splines[s_ind].cpFuncs[2])[1]))\
