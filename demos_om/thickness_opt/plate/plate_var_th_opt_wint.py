...
nonmatching_opt_ffd.set_thopt_surf_inds_FFD(list(range(num_surfs)))
...
xi_end = nonmatching_opt_ffd.splines[-1].parametricCoordinates()
...
residuals = []
for i in range(num_surfs):
    source_terms += [inner(f_list[i], nonmatching_opt_ffd.splines[i].rationalize(
        nonmatching_opt_ffd.spline_test_funcs[i]))*nonmatching_opt_ffd.splines[i].ds]
...
cp_ffd_lims = nonmatching_opt_ffd.thopt_cpsurf_des_lims
for field in [2]:
...
nonmatching_opt_ffd.set_thopt_FFD(FFD_block.knots, FFD_block.control)
nonmatching_opt_ffd.set_thopt_align_CPFFD(align_dir=1)
nonmatching_opt_ffd.set_thopt_regu_CPFFD([None], [None])

# Set up optimization
if save_files:
    nonmatching_opt_ffd.create_files(save_path=save_path, folder_name=folder_name, 
                                 thickness=nonmatching_opt_ffd.opt_thickness)
model = VarThOptGroup(nonmatching_opt_ffd=nonmatching_opt_ffd)
...
for i in range(num_surfs):
    for xi_ind in range(num_pts):
        h_th_profile += [nonmatching_opt_ffd.h_th[i]((xi0_array[xi_ind], xi1))]
