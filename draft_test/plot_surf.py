...
# s0_control[...,2][1] += np.ones(npts)*(1.)
s0_control[...,2][1][1] += 0.5 #np.ones(npts)*(.5)
s0_control[...,2][1][0] += 0.2
s0_control[...,2][1][2] += 0.2
s0_control[...,2][0][1] += 0.2
s0_control[...,2][2][1] += 0.2

surfs_new += [NURBS(surfs[0].knots, s0_control)]

# # Elevate surface
# surfs_new[0].elevate(0, 1)
# surfs_new[0].elevate(1, 1)

# ##########################################
# # Refine surface
# # new_knots0 = np.array([0.125, 0.25, 0.375, 0.625, 0.75, 0.875])
# new_knots0 = np.array([0.125, 0.25 , 0.375, 0.5  , 0.625, 0.75 , 0.875])
# surfs_new[0].refine(0, new_knots0)
# surfs_new[0].refine(1, new_knots0)
# ##########################################

# s1_control = surfs[1].control
# s1_control[...,0][1] += np.ones(npts)*(-0.1)
# s1_control[...,0][2] += np.ones(npts)*(-0.1)
...
for i in range(len(num_els)):
    vtk_writer.write('./nm_geom/surf'+str(i)+'_ref.vtk', surfs_new[i], ref_level=8)
    vtk_writer.write_cp('./nm_geom/surf'+str(i)+'_cp_ref.vtk', surfs_new[i])
...
for i in range(len(surfs)):
        ...
        splines += [spline,]
        File("./nm_geom/para_mesh_ref"+str(i)+".pvd") << splines[i].mesh