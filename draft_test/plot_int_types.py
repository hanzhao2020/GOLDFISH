...
y_off = 0.
z_off = 0.
pts1 = np.array([[x_off, 0.+y_off, 0+z_off], 
                 [x_off, 1.+y_off, 0+z_off],
                 [x_off, 0.+y_off, -0.5+z_off],
                 [x_off, 1.+y_off, -0.5+z_off]])
pts_list = [pts0, pts1]
...
s0_control[:,1,:][:,2] += np.ones(npts)*(0.3)

# s0_control[...,2][0,1] += np.ones(npts)*(0.3)
# s0_control[...,2][2,1] += np.ones(npts)*(-0.3)
...
s1_control[1][0,2] += 0.3 #np.ones(npts)*(0.1)
s1_control[1][2,2] += -0.3 #np.ones(npts)*(0.1)
# s1_control[:,1][:,0] += 0.2 #np.ones(npts)*(0.1)
...
for i in range(len(num_els)):
    vtk_writer.write('./nm_geom/surf'+str(i)+'_init.vtk', surfs_new[i], ref_level=7)
...
preprocessor.compute_intersections(mortar_nels=[12])
